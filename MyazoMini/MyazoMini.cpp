...
//グローバル変数
const wchar_t* Title=L"MyazoMini",
	*WindowClass=L"MYAZOMINIWIN",
	*WindowClassL=L"MYAZOMINIWINL",
	*TempPrefix=L"myamini",
	*UploadServer=L"myazo.net",
	*ScriptName=L"upload.php";
...
void RegisterWindowClass(HINSTANCE Instance)
{
	...
	WndClass.hCursor=(HCURSOR)Cursor.get();//十字カーソル
...
bool UploadFile(const std::wstring& FileName,bool ShowBrowser)
{
	...
	//User-Agentを指定
	const std::wstring UserAgent=L"User-Agent: MyazoMini_win/1.00\r\n";
	...
	if(HttpSendRequest(RequestHandle.get(),Header.c_str(),Header.length(),(void*)OutputMessage.c_str(),OutputMessage.length())){
		...
		if(_wtoi(StatusCode.data())!=200){
			...
			//そんなに長いことはないけどまあ一応
			while(InternetReadFile(RequestHandle.get(),(void*)ResultBuffer.data(),ResultBuffer.size(),&Length)&&Length)
				ResultString.append(ResultBuffer.data(),Length);
